...
# usage ./createVM [subscriptionId] [resourceGroupName] [resourceName] [adminUserName] [adminPublicKey] [imageName]
#==================================
...
parameters=`cat parameters.json | sed -e 's"__resourceName__"'${resourceName}'"' -e 's"__adminUserName__"'${adminUserName}'"' -e 's"__adminPublicKey__"'${escapePublicKey}'"' -e 's"__imageName__"'${imageName}'"' -e 's"{space}"\ "g'`
